...
<div class="card" x-data="DataTable({
        ...
        <div class="data-table-toolbar">
            ...
            <div class="data-table-search">
                ...
                <input type="search" class="form-control form-control-sm" placeholder="{{ $searchPlaceholder }}"
                    x-model="q" @input.debounce.250ms="rechercher">
            </div>
            ...
            @if ($showFilters && $filters ?? false)
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu p-3" style="min-width: 280px">
                        {{ $filters }}
                        <hr class="my-2">
                        <button type="button" class="btn btn-sm btn-uvci w-100"
                            @click="filtresColonnes = {}; document.querySelectorAll('.dt-filter-select').forEach(s => s.value = ''); rechercher()">
    ...
    {{-- Pagination --}}
    <div class="card-footer bg-white border-top dt-footer" x-show="!info.vide">

        <div class="dt-pagination">
            <small class="text-muted" x-text="info.pagination"></small>

            <div class="dt-pages" x-show="info.pages > 1">
                <button class="dt-page" :class="{ 'dt-disabled': page === 1 }" @click="page > 1 && (page--, afficher())"
                    :disabled="page === 1" aria-label="Précédent">
                    <i class="fa-solid fa-chevron-left"></i>
                </button>

                <template x-for="(p, i) in info.visibles" :key="i">
                    <button class="dt-page" :class="{ 'dt-active': p === page, 'dt-disabled': p === '…' }"
                        x-text="p" @click="typeof p === 'number' && (page = p, afficher())"
                        :disabled="p === '…'">
                    </button>
                </template>

                <button class="dt-page" :class="{ 'dt-disabled': page === info.pages }"
                    @click="page < info.pages && (page++, afficher())" :disabled="page === info.pages"
                    aria-label="Suivant">
    ...
    <script>
        function DataTable({
            ...
            return {
                ...
                get filtrees() {
                    let data = this.lignes.map(l => ({
                        el: l,
                        texte: l.textContent.toLowerCase(),
                        cellules: Array.from(l.querySelectorAll('td')).map(td => td.textContent.trim()
                            .toLowerCase())
                    }));
                    ...
                    Object.entries(this.filtresColonnes).forEach(([colIndex, valeur]) => {
                        if (valeur) {
                            const idx = parseInt(colIndex);
                            data = data.filter(d => {
                                const cellText = d.cellules[idx] || '';
                                return cellText.includes(valeur.toLowerCase());
                            });
                        }
                    });
                    ...
                    if (this.triCol !== null) {
                        data.sort((a, b) => {
                            const va = (a.el.querySelectorAll('td')[this.triCol]?.textContent.trim()
                                .toLowerCase() ?? '');
                            const vb = (b.el.querySelectorAll('td')[this.triCol]?.textContent.trim()
                                .toLowerCase() ?? '');
                ...
                filtrerColonne(index, valeur) {
                    this.filtresColonnes[index] = valeur;
                    this.page = 1;
                    this.afficher();
                },
        ...
        function filtrerDataTable(colIndex) {
            ...
            alpine.filtrerColonne(colIndex, val);
        }
    </script>
